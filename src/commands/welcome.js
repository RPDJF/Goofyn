...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const welcomeDoc =
      (await db.getData("guilds", interaction.guildId)).welcome ||
      { enabled: false, channel_id: null, message: null, thumbnail_url: null };
    let update = false;
    ...
    if (!groupcommand) {
      if (subcommand === "enable") {
        ...
        if (!welcomeDoc.message || !welcomeDoc.channel_id) {
          ...
          const description =
            dictionary.commands.welcome.errors.no_channel_or_message;
          await interaction.editReply({
            embeds: [errorMsg(title, description)],
            ephemeral: true,
          });
        ...
        const title = enable
          ? dictionary.commands.welcome.title.enabled
          : dictionary.commands.welcome.title.disabled;
        const description = dictionary.commands.welcome.success.toggle.replace(
          "{value}",
          enable,
        );
        await interaction.editReply({
          embeds: [msg(title, description)],
          ephemeral: true,
        });
        await new Promise((resolve) => setTimeout(resolve, 10000)).then(
          async () => await interaction.deleteReply(),
      ...
      if (subcommand === "channel") {
        ...
        const description = dictionary.commands.welcome.success.channel.replace(
          "{channel}",
          channel_id,
        );
        ...
        if (
          ...
          thumbnail_url !== welcomeDoc.thumbnail_url
        ) {
  ...
  async onGuildMemberAdd_hook(member) {
    ...
    if (
      !guildDoc || guildDoc.welcome.enabled || !guildDoc.welcome.channel_id ||
      !guildDoc.welcome.message
    ) {
      return;
    }
    ...
    await channel.send({
      content: message,
      files: guildDoc.welcome.thumbnail_url
        ? [guildDoc.welcome.thumbnail_url]
        : [],
    });
  },
};
