const { Interaction, SlashCommandBuilder, PermissionFlagsBits, GuildMember } =
  require("discord.js");
    ...
    if (subcommand === "add") {
      ...
      await interaction.editReply({
        embeds: [msg(
          ...
          dictionary.commands.autorole.success.remove.replace(
            "{role}",
            removeRole,
          ),
        )],
      ...
      if (roles && roles.length > 0) {
        ...
        await interaction.editReply({
          embeds: [
            errorMsg(
              dictionary.errors.title,
              dictionary.commands.autorole.errors.empty_roles,
            ),
          ],
          ephemeral: true,
        });
