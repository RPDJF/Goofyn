...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => {
      option.setName("language")
        ...
        .setRequired(true);
      const choices = Object.keys(langCode).map(
        (lang) => [{ name: langCode[lang].data.name, value: lang }],
      );
  ...
  async execute(interaction) {
    ...
    let lang = interaction.guild
      ? await db.getData("guilds", interaction.guildId).lang
      : await db.getData("users", interaction.user.id).lang;
    ...
    if (langCode[newLang] === undefined) {
      ...
      await interaction.editReply({
        embeds: [
          errorMsg(
            ...
            dictionary.commands.language.errors.invalid,
          ),
        ],
        ephemeral: true,
      });
      ...
      if (lang !== newLang) {
        await db.writeData(
          interaction.guild ? "guilds" : "users",
          interaction.guild ? interaction.guildId : interaction.user.id,
          { lang: newLang },
        );
      }
      dictionary = await getDictionary(
      ...
      await interaction.editReply({
        embeds: [
          msg(
            dictionary.success,
            dictionary.commands.language.success,
            dictionary.author,
          ),
        ],
        ephemeral: true,
