    ...
    try {
      ...
      if (subcommand === "character") {
        const character = await jikanClient.getCharacterFull(
          (await jikanClient.getCharacters({
            limit: 1,
        ...
        let voices = character.voices.length
          ...
          : null;
        if (voices && voices.length > 1024) {
          voices = voices.substring(0, 1024 - "...".length) + "...";
        }
        ...
        const anime = await jikanClient.getAnimeFull(
          (await jikanClient.getAnimes({
            limit: 1,
            q: interaction.options.getString("name"),
            order_by: "favorites",
            sort: "desc",
        ...
        const title = anime.titles[0].title + anime.year
          ? ` (${anime.year})`
          : "";
