const { Interaction, SlashCommandBuilder, PermissionFlagsBits, EmbedBuilder } =
  require("discord.js");
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === "new") {
      const embed_title = await textParser(
        interaction,
        interaction.options.getString("title"),
      );
      const embed_description = await textParser(
        interaction,
        interaction.options.getString("description"),
      );
      const embed_color = interaction.options.getString("color")
        ? interaction.options.getString("color").replace("#", "").replace(
          "0x",
          "",
        )
        : undefined;
      const embed_image = interaction.options.getString("image") || undefined;
      const embed_thumbnail = interaction.options.getString("thumbnail") ||
        undefined;
      ...
      if (embed_description) {
        embed_description = await textParser(interaction, embed_description);
      }
      const embed_color = interaction.options.getString("color")
        ? interaction.options.getString("color").replace("#", "").replace(
          "0x",
      ...
      if (!messageToEdit) {
        ...
        if (!embed_title || !embed_description) {
          const title = dictionary.errors.title;
          const description =
            dictionary.commands.embed.errors.embed_no_title_description;
          await interaction.editReply({
            embeds: [errorMsg(title, description)],
            ephemeral: true,
          });
      ...
      const messageToRemove = await interaction.channel.messages.fetch(
        messageID,
      );
      if (!messageToRemove) {
        const title = dictionary.errors.title;
        ...
        !messageToRemove.embeds || messageToRemove.embeds.length <= 0
      ) {
        const title = dictionary.errors.title;
