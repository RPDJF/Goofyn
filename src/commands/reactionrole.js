  ...
  if (reactionroles && reactionroles[0]) {
    ...
    for (const reactionrole of reactionroles) {
      try {
        ...
        reactionroles = reactionroles.filter((r) =>
          r.message_id !== reactionrole.message_id
        );
        triggered = true;
      }
    }
    if (triggered) {
      await db.writeData("guilds", interaction.guildId, {
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let reactionroles =
      (await db.getData("guilds", interaction.guildId)).reactionrole;
    ...
    if (subcommand === "new") {
      ...
      const messageData = {
        embeds: [
          msg(message, dictionary.commands.reactionrole.messages.choose_role),
        ],
        ephemeral: false,
      };
      await interaction.channel.send(messageData).then(async (message) => {
        ...
        db.writeData("guilds", interaction.guildId, {
          reactionrole: reactionroles,
        }, true);
      });
      ...
      if (
        !emoji.match(/<a?:.+>/) && !emoji.match(/\p{Extended_Pictographic}/u)
      ) {
        await interaction.editReply({
          embeds: [
            errorMsg(
              dictionary.errors.title,
              dictionary.commands.reactionrole.errors.invalid_emoji,
            ),
          ],
          ephemeral: true,
        });
      ...
      if (reactionroles && reactionroles[0]) {
        ...
        if (reactionrole) {
          ...
          const updatedDescription =
            `${embed.description}\n\n${emoji} : ${description}`;
          const updatedEmbed = new EmbedBuilder(embed).setDescription(
            updatedDescription,
          );
          ...
          await interaction.editReply({
            embeds: [
              msg(
                dictionary.commands.reactionrole.title.add,
                dictionary.commands.reactionrole.messages.added.replace(
                  "{role}",
                  role.id,
                ),
              ),
          ...
          await interaction.editReply({
            embeds: [
              errorMsg(
                dictionary.errors.title,
                dictionary.commands.reactionrole.errors.message_not_found,
              ),
            ],
            ephemeral: true,
          });
        ...
        await interaction.editReply({
          embeds: [
            errorMsg(
              dictionary.errors.title,
              dictionary.commands.reactionrole.errors.no_reactionroles,
            ),
          ],
          ephemeral: true,
        });
      ...
      const emoji = interaction.options.getString("emoji");
      if (reactionroles && reactionroles[0]) {
        let reactionrole_message = reactionroles.find((r) =>
          r.message_id === message_id
        );
        let reactionrole;
        if (reactionrole_message) {
          reactionrole = reactionrole_message.roles.find((r) =>
            r.emoji === emoji
          );
        }
        if (reactionrole) {
          const message = await (await interaction.guild.channels.fetch(
            reactionrole_message.channel_id,
          )).messages.fetch(reactionrole_message.message_id);
          const reaction = message.reactions.cache.find((r) =>
            r.emoji.id === reactionrole.reaction_id ||
            r.emoji.name === reactionrole.reaction_id
          );
          ...
          await message.edit({ embeds: [updatedEmbed] });
          reactionrole_message.roles = reactionrole_message.roles.filter((r) =>
            r.emoji !== emoji
          );
          await db.writeData("guilds", interaction.guildId, {
            reactionrole: reactionroles,
          ...
          await interaction.editReply({
            embeds: [
              msg(
                dictionary.commands.reactionrole.title.remove,
                dictionary.commands.reactionrole.messages.removed.replace(
                  "{role}",
      ...
      let reactionroles =
        (await db.getData("guilds", interaction.guildId)).reactionrole;
      if (reactionroles && reactionroles[0]) {
        const findReactionRole = reactionroles.find((r) =>
          r.message_id === message_id
        );
        if (findReactionRole) {
          ...
          await interaction.editReply({
            embeds: [
              msg(
                dictionary.commands.reactionrole.title.delete,
                dictionary.commands.reactionrole.messages.deleted,
              ),
            ],
            ephemeral: true,
          });
      ...
      if (reactionroles && reactionroles[0]) {
        ...
        for (const reactionrole of reactionroles) {
          message +=
            `https://discord.com/channels/${interaction.guildId}/${reactionrole.channel_id}/${reactionrole.message_id}\n`;
        ...
        interaction.editReply({
          embeds: [msg(dictionary.commands.reactionrole.title.list, message)],
          ephemeral: true,
        });
      } else {
        await interaction.editReply({
  ...
  async onReactionAdd_hook(reaction, user) {
    ...
    let reactionroles =
      (await db.getData("guilds", reaction.message.guild.id)).reactionrole;
    if (reactionroles && reactionroles[0]) {
      const reactionrole = reactionroles.find((r) =>
        r.message_id === reaction.message.id &&
        r.channel_id === reaction.message.channel.id
      );
      if (reactionrole) {
        let role = reactionrole.roles.find((r) =>
          r.reaction_id === reaction.emoji.id ||
          r.reaction_id === reaction.emoji.name
        );
        if (role) {
            ...
            if (err.code === 50013) {
              ...
              const reply = await user.send({
                embeds: [
                  errorMsg(
                    dictionary.errors.title,
                    dictionary.errors.execution_missing_perms,
                  ),
                ],
                ephemeral: true,
              });
