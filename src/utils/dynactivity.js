...
async function dynactivity(client) {
  ...
  setInterval(async () => {
    if (index === 0) {
      ...
      const userCount = guildsFetch.reduce(
      ...
      client.user.setActivity(`${nbCommands} Commands 🤖`, {
        type: ActivityType.Custom,
      });
    } else if (index === 3) {
      client.user.setActivity(`You can talk to me! 🗣️`, {
        type: ActivityType.Custom,
      });
      await new Promise((resolve) => setTimeout(resolve, 2000));
    } else if (index === 4) {
      client.user.setActivity(`Open source! 🚀`, {
        type: ActivityType.Custom,
        url: "https://github.com/rpdf/goofyn.git",
      });
    }
    index++;
    if (index >= 5) {
