...
async function textParser(context, text) {
  ...
  if (text.includes("{membercount}")) {
    const memberCount = context.guild
      ? (await context.guild.members.fetch()).size
      : "undefined";
    output = output.replace(/{membercount}/g, memberCount);
  }
  if (text.includes("{rolecount}")) {
    const roleCount = context.guild
      ? (await context.guild.roles.fetch()).size
      : "undefined";
  ...
  output = output
    .replace(
      /{user}/g,
      `<@${context.user ? context.user.id : context.author.id}>`,
    )
    .replace(
      /{username}/g,
      context.user ? context.user.username : context.author.username,
    )
    ...
    .replace(
      /{channel}/g,
      context.channelId ? `<#${context.channelId}>` : "undefined",
    )
